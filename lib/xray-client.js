...
const segmentParser = function(s) {
    ...
    if (document.origin === LAMBDA_SERVICE_DOC_ORIGIN || document.origin === LAMBDA_FN_DOC_ORIGIN) {
        ...
        // Capture if the segment was an async lambda invocation
        if (document.http && document.http.response && document.http.response.status === 202) {
            event.async = true;
        }
        ...
        // Capture lambda service dwell time, if exists.
        if (document.origin === LAMBDA_SERVICE_DOC_ORIGIN) {
            if (document.subsegments) {
                document.subsegments.forEach(function(ss) {
                    if (ss.name === "Dwell Time") {
                        event.dwellTime = ss.end_time - ss.start_time;
                    }
                });
            } else {
                event.dwellTime = 0;
...
exports.convertTracesToEvents = function (ids, cb) {
    ...
    // TODO: support multiple via pagination
    var params = {
    ...
    xray.batchGetTraces(params, function (err, data) {
        ...
        data.Traces.forEach(function(t) {
            t.Segments.forEach(function(s) {
                ...
                //log('[parsed segment] ', JSON.stringify(segment));
                if (segment !== null) {
